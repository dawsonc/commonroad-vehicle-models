...
def vehicle_dynamics_mb(x, uInit, p):
    ...
    dot_delta_f = (
        ...
        + dot_delta_z_f * math.sin(x[6])
        - dot_delta_y_f * math.cos(x[6])
        - (p.h_raf - p.R_w) * math.cos(delta_phi_f) * dot_delta_phi_f
    )
    dot_delta_r = (
        (delta_z_r * math.cos(x[6]) + x[28] * math.sin(x[6])) * x[7]
        + dot_delta_z_r * math.sin(x[6])
        - dot_delta_y_r * math.cos(x[6])
    ...
    sumN = (
        ...
        + (F_x_LF + F_x_RF) * p.a * math.sin(x[2])
        + (F_y_RF - F_y_LF) * 0.5 * p.T_f * math.sin(x[2])
