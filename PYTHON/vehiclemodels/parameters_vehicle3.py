...
def parameters_vehicle3():
    ...
    p.m_uf = unitConversion.lb_sec2_ft_IN_kg(
        5.56015
    ...
    # suspension parameters
    p.D_f = unitConversion.rad_ft_IN_rad_m(
        -0.1
    )  # [rad/m]  DF; modified from orig. paper due to stability issues when damping is ignored
    p.D_r = unitConversion.rad_ft_IN_rad_m(
        -0.1
    )  # [rad/m]  DR; modified from orig. paper due to stability issues when damping is ignored
